...
要求：
1. summary 要簡短清楚。
2. knowledge_points 請產生 5 到 8 個。
3. questions 請產生 5 到 10 題選擇題。
4. 每題都要有四個選項 A、B、C、D。
5. answer 只能是 A、B、C、D。
6. 如果逐字稿沒有時間點，start_time 和 end_time 請填 null。
7. 不要輸出 markdown，不要輸出程式碼區塊，只要純 JSON。
8. 請確保輸出是合法 JSON。
